...
-- Existing user-scoped substrate rows are Luca's; new rows can belong to any
-- user-owned agent. This migration is additive/backfill-first except for
-- replacing one-row-per-user uniqueness with one-row-per-user-agent uniqueness.
