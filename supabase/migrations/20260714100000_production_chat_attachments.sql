...
-- Serialize each user's attachment initialization so concurrent requests
-- cannot race past the per-turn count or storage quota enforced by the API.
...
-- Lifecycle mutation is deliberately edge-function/service-role only.
...
-- Associate records in the same transaction as message persistence. This is
-- the durable boundary used by retries and deletion cascades: inference never
-- begins with attachment IDs that exist only in client state.
...
begin
  ...
  -- Account restores insert messages before their attachment rows because the
  -- attachment foreign key points back to the message. The canonical rows are
  -- inserted immediately afterwards with message_id already restored. Any
  -- partially-existing set still fails closed.
...
-- Backfill direct-chat legacy JSON records when a private object path exists.
with legacy as (
...
-- Backfill the existing group-room shape without moving the underlying object.
...
-- Install quota enforcement only after the lossless legacy backfill. Existing
-- user history must remain portable even when it predates current plan limits.
...
-- Reads are issued as short-lived signed URLs by authenticated edge functions.
