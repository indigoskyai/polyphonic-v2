...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE p.id = NEW.project_id AND p.user_id = NEW.user_id AND p.archived = false
  ) THEN
...
CREATE INDEX IF NOT EXISTS projects_user_active_idx ON public.projects(user_id, archived, pinned DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS threads_user_project_idx ON public.threads(user_id, project_id, updated_at DESC);
