...
-- Social account connections (Moltbook, X)
...
-- Policies (user select + service role full)
...
-- Indexes
CREATE INDEX idx_activity_log_user_created ON entity_activity_log(user_id, created_at DESC);
CREATE INDEX idx_task_queue_user_status ON entity_task_queue(user_id, status);
