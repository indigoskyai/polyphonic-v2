...
Deno.serve(async (req) => {
  ...
  return wrapHandler(corsHeaders, async () => {
    ...
    try {
      const ready = await finalizeAttachmentRecord(ctx.admin, attachment, body.extraction);
      return jsonResponse({ attachment: descriptorFromRow(ready), ready: true }, corsHeaders);
    } catch (finalizeError) {
      const failed = await recordAttachmentFailure(ctx.admin, id, finalizeError);
      if (!failed) throw finalizeError;
