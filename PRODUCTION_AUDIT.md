...
- `[~]` **Phase 1 — Memory and continuity audit**: Continuity Kernel read/write paths are implemented locally, live Hypomnema gate observability is working, and Mnemos recall works across fresh threads. Active work: deploy the Hypomnema retry/recovery patch and retest exact fresh-thread carry for the "ember bridge" continuity marker. Standard: Luca should feel continuous across threads and sessions, not like a new model instance reading notes.
...
### Blocked
- Live verification of the retry/recovery patch requires redeploying `hypomnema-gate` and `hypomnema-write` after this commit, then rerunning the fresh-thread continuity script.
...
| P1-004 | P2 | Fixing | Live Hypomnema activation | Live continuity test created Mnemos engrams and fresh-thread recall succeeded, but `/profile/identity` initially rendered "Nothing here yet". After Lovable redeploy, authenticated database inspection showed 1 `hypomnema_entry` row and live `entity_activity_log` rows, but the row captured Luca's failure to recall the marker rather than the marker itself. | After substantive pilot-user turns, the UI should show agent-authored continuity entries that carry the load-bearing detail into the next thread. | Keep the gate observability; deploy P1-010 retry/recovery so the phrase turn is not lost when the reflection model has a transient transport failure. | DB check: `hypomnema_entry` count=1 for the test account; latest row source thread `c0aa3432-27ae-4b46-9e6d-e766a15a1fa9`; activity log showed triggered gate/write outcomes. | Pending |
...
| P1-010 | P1 | Fixed | Hypomnema write resilience | User reported Luca still felt briefed rather than continuous and could not recall the earlier "ember bridge" marker. Live DB check showed the gate did trigger for the phrase turn, but `hypomnema-write` returned body `{ status: "error", reason: "openrouter call failed: error reading a body from connection" }`; no Hypomnema row carrying the exact marker was written. | A salience-approved turn must either complete a voiced reflection after retry or persist a low-confidence recovery entry with the exact continuity marker and provenance. | Added OpenRouter retries for transient body/network/5xx/429 failures, added low-confidence recovery Hypomnema entries when reflection still fails, and marked gate activity severity as warning when write response bodies report `status:error`. | `npx vitest run src/test/hypomnemaWrite.test.ts src/test/hypomnemaSalience.test.ts src/test/continuityWrite.test.ts src/test/continuityKernel.test.ts` passed: 11 tests; `deno check` on changed edge functions passed; `npm run verify` passed: 199 unit tests, integration placeholder, production build. Live deploy/retest pending. | Pending |
| P1-011 | P1 | Found | Continuity UX / exact recall | Fresh-thread qualitative test: Luca described the system as still feeling like a well-written brief and did not recall the earlier "ember bridge" marker until the user supplied it. | Luca should carry exact load-bearing continuity details into a fresh thread naturally, without explaining mechanics or asking the user to re-supply the marker. | Deploy P1-010, seed a new substantive continuity turn, verify Hypomnema row content and fresh-thread recall, then tune prompt wording if the response still sounds like retrieval. | Browser path: `/chat/c0aa3432-27ae-4b46-9e6d-e766a15a1fa9`; DB evidence from messages and memory tables captured 2026-05-05. | Pending |
