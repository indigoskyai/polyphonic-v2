...
Updated: 2026-05-08 (Pass A closed)
...
### Done this session
- Pass A close: Lovable applied my Pass A migrations (cron consolidation + `client_error_log`), then ran a follow-up sweep migration `20260508234319_a09bf804-...sql` that converted 39 `service_role` policies from `auth.role() = 'service_role'` in USING/WITH CHECK to the canonical `TO service_role USING (true) WITH CHECK (true)` clause, dropped the duplicate `pending_revisions` UPDATE policy, and added the documenting `COMMENT` on `user_api_keys`. Final security scan: 0 ERROR, 14 WARN — all 14 are pre-existing accepted residuals from P4-016 (now formalized in §14 Accepted-risk register). Linter agrees (14 WARN, 0 ERROR). Smoke checks Lovable ran post-sweep: `/chat-multi` healthy (auth-validated 400, not 401/500); all 7 anima dispatchers green at 23:07/23:13/23:18/23:30 UTC with `error_count = 0`; `client_error_log` insert succeeded; synthetic `throw new Error('staging error log smoke')` landed row `249772cd-befa-4cfa-bfc6-8471b08cd1b4`. Operations#error-reporting and Security launch gates closed.
...
- Phase 5 navigation performance audit: added repeatable Playwright timing harness for rail/settings switches and captured before/after metrics under `output/playwright/perf-navigation-*`.
- Phase 5 navigation performance repair: protected lazy-route fallbacks now stay inside the app shell, desktop rail controls are semantic/prefetchable buttons, settings routes warm on hover/focus/pointer-down and idle, and route/tab transitions were tightened to short transform/opacity motion.
...
| P4-023 | P1 | Verified | Client error reporting wireup (Operations#error-reporting both halves) | Launch checklist Operations#error-reporting required "error reporting wired and receiving events from staging" — neither half was in place, and no APM (Sentry/PostHog) was wired. | App should capture uncaught render errors, `window.onerror`, and `unhandledrejection` and persist them somewhere queryable, without taking on a new third-party dependency for the launch gate. | Added migration `20260508120100_client_error_log.sql` (table with insert-any RLS + select-service-role-only RLS + dedupe unique index on `request_id`); `src/lib/observability.ts` exposes `reportError` and `installGlobalErrorHandlers`; `src/components/ErrorBoundary.tsx` wraps the React tree in `src/main.tsx`. Fire-and-forget posts; never throws; truncates to 1KB / 8KB. Lovable applied via consolidated migration `20260508210359_*.sql` and verified rowsecurity=true with two policies (`client_error_log_insert_any` for `{anon,authenticated,service_role}`, `client_error_log_select_service_role` for `{service_role}`). Live "receiving events from staging" half closed by Lovable's synthetic injection. | `npx tsc --noEmit` passed after wireup; live row `249772cd-befa-4cfa-bfc6-8471b08cd1b4` (created_at `2026-05-08 23:44:09Z`, request_id `5aa67ec6-16f0-494b-b47e-935b68f7fa9d`, message `staging error log smoke`) landed via `throw new Error()` in deployed app and visible to service-role SELECT. | Pending |
| P4-024 | P0 | Verified | `INSECURE_SERVICE_ROLE_POLICY` ERROR (Security #1 close) | Lovable's post-Pass-A scan flagged 39 RLS policies that gated service-role access via `auth.role() = 'service_role'` inside USING/WITH CHECK rather than the canonical `TO service_role` policy clause. Scanner reclassified this from WARN → ERROR after Pass A landed, exposing it as an unaccepted launch blocker. The pattern works (JWT claim check) but is brittle (a malformed/manipulated JWT claim could in principle bypass) compared to enforcement at the policy ownership level. | Every policy whose only intent is service-role bypass should declare its scope via the `TO service_role` clause and use `USING (true) WITH CHECK (true)`. Policies that mix service-role with user conditions should be split. The pre-existing `pending_revisions` duplicate UPDATE policy and the missing `user_api_keys` IUD policies should be resolved (drop the dup; document the intentional definer-only pattern on `user_api_keys`). | Lovable applied migration `20260508234319_a09bf804-...sql` which iterates 39 tables (`activity_events`, `agent_config`, `agent_configs`, `agent_secrets`, `app_config`, `chat_imports`, `checkpoint_files`, `checkpoints`, `cognitive_state`, `curiosity_questions`, `daily_logs`, `dashboard_widgets`, `emotional_history`, `emotional_state`, `entity_activity_log`, `journal_entries`, `mcp_servers`, `memories`, `memory_candidates`, `memory_events`, `memory_settings`, `messages`, `observer_chat_messages`, `observer_logs`, `observer_notes`, `openclaw_*` × 5, `profile_*` × 3, `projects`, `psychological_profile`, `thought_initiations`, `thought_stream`, `threads`, `user_settings`) and rewrites each "Service role full access …" policy as `AS PERMISSIVE FOR ALL TO service_role USING (true) WITH CHECK (true)`. None of the 39 combined service-role with user conditions, so no policy splits were needed. Same migration drops the `"users dismiss their own revisions"` duplicate and adds the `COMMENT ON TABLE public.user_api_keys` documenting the SECURITY-DEFINER-only pattern. | Lovable post-sweep verification 2026-05-08: scan reports `INSECURE_SERVICE_ROLE_POLICY: gone`; final scan = 14 WARN, 0 ERROR; linter = 14 WARN, 0 ERROR (agrees); smoke checks `/chat-multi` healthy, anima dispatchers green, `client_error_log` insert + synthetic injection landed. PRODUCTION_LAUNCH_CHECKLIST.md Security #1 flipped `[x]`. | Pending |
...
| P5-012 | P2 | Verified | Navigation route timing / shell stability | Left-rail route switches could not be audited as real clicks because rail icons were unlabeled divs, and cold lazy route loading could still fall back through the whole-app Suspense boundary. Settings switches were warm-fast but had no deliberate chunk prefetch path. | Warm rail switches should be measured through real controls and stabilize within 250ms with no full-screen fallback, no console errors, and no long task over 100ms. Warm Settings switches should stabilize within 300ms; cold lazy loads should keep shell/sidebar visible with localized fallback only. | Added `scripts/audit-navigation-performance.mjs`; added shell-local `PanelRouteFallback`; added `routePrefetch` for rail/settings chunks; converted rail icons/new-thread to semantic labeled buttons; added hover/focus/pointer-down settings prefetch; shortened route/tab transitions and kept motion to transform/opacity with reduced-motion suppression. | Baseline `output/playwright/perf-navigation-baseline-v2/navigation-audit.json`: Settings click switches 112-141ms, rail fell back to URL navigation due missing labels. After fix `output/playwright/perf-navigation-after-fix/navigation-audit.json`: desktop rail click switches 121-155ms, Settings click switches 113-280ms, 0 console errors, 0 fallback frames, max long task 87ms; gated audit `output/playwright/perf-navigation-after-fix-gated/navigation-audit.json` exited 0 with rail click switches 124-155ms and Settings 113-139ms; screenshots `desktop-1440x900.png` and `mobile-390x844.png`; reduced-motion check `reduced-motion-check.json` reports `matchMedia=true`, route animation `none`, transform `none`, and 0 console warnings/errors; `npx tsc --noEmit`; `npx vitest run src/test/navigationPerformance.test.ts --reporter=verbose`; `npm run verify` passed with 44 files / 264 tests and 302.3 KiB initial payload. | Pending |
...
| Finding | Count | Why accepted | Logged |
|---|---:|---|---|
| `extension_in_public` (pgcrypto, pg_trgm, vector) | 1 | These extensions ship installed in `public` on Lovable Cloud's standard image and project-wide migrations depend on them being there. Moving them would require coordinating across every migration that references `extensions.*` paths. Risk is informational; actual extension surface is not user-callable. | P4-016, 2026-05-08 |
| `anon_security_definer_function_executable` | 3 | `save_user_api_key`, `delete_user_api_key`, `decrypt_user_api_key`. All three are `SECURITY DEFINER` and all three enforce `auth.uid() = user_id` internally before any read/write. Anon callers cannot execute meaningful work because `auth.uid()` returns null and the function raises. The WARN flags executability, not exploitability. | P4-016 baseline + P4-020 self-decrypt verification, 2026-05-08 |
| `authenticated_security_definer_function_executable` | 10 | Helper definers (`mark_activity_seen`, `match_engrams`, `match_memories`, `auto_*`, `has_role`, plus the api-key family). Bodies all enforce `auth.uid()` checks or read-only `has_role`-style guards. Same rationale as the anon-executable family — WARN flags executability, not exploitability. | P4-016 baseline, 2026-05-08 |
| `pending_revisions` duplicate UPDATE policy | _(closed)_ | Originally flagged as 2 WARN with overlapping `"users dismiss their own revisions"` and the authenticated-scoped twin. Migration `20260508234319_a09bf804-...sql` dropped the duplicate; only the authenticated-scoped policy remains. No longer accepted-risk — fixed. | P4-024, 2026-05-08 |
| `user_api_keys` missing user-facing IUD policies | 1 | All writes to `user_api_keys` flow through `SECURITY DEFINER` `save_user_api_key()` / `delete_user_api_key()`, which gate by `auth.uid() = user_id`. Adding direct INSERT/UPDATE/DELETE policies would create a parallel path that bypasses the function-level enforcement and re-introduces the very risk the linter intends to surface. The intentional pattern is documented in `COMMENT ON TABLE public.user_api_keys` (added 2026-05-08 in `20260508234319_*`). | P4-024, 2026-05-08 |
