...
- `[~]` **Phase 1 — Memory and continuity audit**: Continuity Kernel read/write paths are implemented, live Hypomnema writes are landing, revision provenance is verified in staging, and fresh-thread recall now carries the "ember bridge" marker. Active work: sanitize exclusion boundaries so user-dropped tangents cannot be named back to Luca through polluted continuity layers. Standard: Luca should feel continuous across threads and sessions, not like a new model instance reading notes.
...
### Blocked
- Live verification of the exclusion redaction fix requires pushing/redeploying `chat` and `chat-multi`, then rerunning the fresh-thread continuity script.
