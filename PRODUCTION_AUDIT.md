...
- Chat UX polish: Observer prose now matches the normal chat body scale, right drawers share a calmer typography hierarchy, and Agent runtime activity renders as integrated rows inside the shared thinking surface.
- Chat motion polish: streaming text now reveals in phrase/word-feeling increments, historical thread messages no longer replay entrance motion, and the first-message handoff remains continuous through thread creation.
- Chat shell polish: composer controls, drawer focus behavior, reduced-motion behavior, and mobile 390/430px containment were verified with deterministic browser evidence in `output/playwright/chat-polish-20260506/`.
...
| CP-CHAT-001 | P2 | Verified | Observer typography | Observer alcove and drawer prose could read as miniature notes beside normal chat messages. | Observer message body, summaries, and ask/response prose should match the normal chat body scale while metadata remains compact. | Raised Observer body classes to the shared chat body scale and moved Observer drawer prose onto shared drawer-copy classes. | Browser smoke compared computed `.a-msg-body` and `.msg-body` font sizes; screenshots `output/playwright/chat-polish-20260506/desktop-observer-alcove.png`; `npm run verify`. | Pending |
| CP-CHAT-002 | P2 | Verified | Right drawer hierarchy | Thread detail, Observer, activity, and agent-dialogue drawers used mixed inline scales and inconsistent body rhythms. | Drawer titles, metadata, body prose, activity rows, and actions should feel like one readable hierarchy. | Added shared drawer prose/section label classes, normalized timeline and metadata typography, and removed ad hoc inline scales in Observer and agent-dialogue drawers. | Screenshot `output/playwright/chat-polish-20260506/desktop-thread-drawer.png`; `npm run verify`. | Pending |
| CP-CHAT-003 | P2 | Verified | Agent activity traces | Persisted Agent activity existed functionally but read like raw thinking text rather than an integrated runtime surface. | Agent runtime/tool activity should appear as calm trace rows inside the thinking surface for live and reloaded messages. | Added shared `ThinkingBlock`, parse-only activity segmentation, lifecycle-colored trace rows, and reused it for streaming and persisted message thinking content. | `npx vitest run src/test/thinkingBlock.test.ts src/test/dialogFocus.test.tsx src/test/reducedMotion.test.tsx`; screenshot `output/playwright/chat-polish-20260506/desktop-agent-trace-expanded.png`; `npm run verify`. | Pending |
| CP-CHAT-004 | P2 | Verified | Streaming text motion | Streaming text could feel like a mechanical character ticker. | Text should arrive in smooth phrase/word-feeling increments without markdown churn or a loud cursor. | Reworked `useSmoothTypewriter` to advance at readable boundaries, throttle state commits, and skip animation for reduced-motion users. | Reduced-motion browser smoke, targeted tests, and `npm run verify`; normal stream behavior remains UI-only and backend contract unchanged. | Pending |
| CP-CHAT-005 | P3 | Verified | Historical message entrance | Existing thread loads and route switches could replay entrance styling meant for new messages. | Historical messages should load stably; only recently appended/sent/streaming rows should animate. | Added a bounded message-entry gate and passed it into message items and special message cards. | Existing-thread desktop/mobile screenshots in `output/playwright/chat-polish-20260506/`; `npm run verify`. | Pending |
| CP-CHAT-006 | P2 | Verified | First-message handoff | The empty-chat first send could briefly feel like a route swap instead of one continuous message gesture. | The optimistic first user row should remain continuous through thread creation and clear only when persisted messages are active. | Kept the staged first-turn handoff through local message insertion and route navigation rather than clearing it immediately after optimistic add. | `npm run verify`; regression logic follows the previously verified first-turn handoff path. | Pending |
| CP-CHAT-007 | P3 | Verified | Composer controls | Send/stop, Agent, Observer, attachment, ensemble, and effort controls needed stable hover/focus/active states. | Control changes should feel immediate and not resize the composer or footer. | Tightened send-button transitions/dimensions, stabilized input-shell focus shimmer, and kept mobile composer controls within safe-area constraints. | Mobile screenshots `mobile-390-existing-thread.png` and `mobile-430-existing-thread-reduced-motion.png`; `npm run verify`. | Pending |
| CP-CHAT-008 | P2 | Verified | Drawers and sheets | Drawer focus and open/close state were partly custom and could expose hidden focusable content risk. | Closed drawers should not be keyboard/AT reachable; open drawers should use shared focus placement, Escape, and restoration behavior. | Wired the shared `useDialogFocus` helper into the right drawer and marked closed drawer content inert/aria-hidden. | `npx vitest run src/test/dialogFocus.test.tsx`; browser smoke opened/closed the thread drawer with Escape; `npm run verify`. | Pending |
| CP-CHAT-009 | P2 | Verified | Reduced motion | Chat streaming, thinking, drawer, and mobile surfaces needed stable behavior under `prefers-reduced-motion`. | Reduced-motion users should get immediate readable state without delayed visibility or animation dependency. | Streaming text now renders directly under reduced motion; changed motion CSS keeps transitions suppressed through existing reduced-motion rules. | Browser smoke at 390x844 and 430x932 with reduced motion; `npx vitest run src/test/reducedMotion.test.tsx`; `npm run verify`. | Pending |
| CP-CHAT-010 | P2 | Verified | Mobile containment | Chat at 390x844 and 430x932 needed proof against horizontal overflow and squeezed desktop artifacts after polish changes. | Mobile chat should remain single-column, readable, and horizontally contained while preserving context controls. | Kept drawer/composer/message changes responsive and verified scroll width against viewport width. | Browser smoke asserted `document.scrollingElement.scrollWidth <= document.documentElement.clientWidth`; screenshots `mobile-390-existing-thread.png`, `mobile-430-existing-thread-reduced-motion.png`; `npm run verify`. | Pending |
