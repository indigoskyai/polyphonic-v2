...
- `[~]` **Phase 1 — Memory and continuity audit**: Continuity Kernel read/write paths are implemented, live Hypomnema writes are landing, revision provenance is verified in staging, fresh-thread recall carries the "ember bridge" marker, and exclusion redaction now passes staging. Active work: inspect remaining Phase 1 write/recall residuals before closing memory continuity. Standard: Luca should feel continuous across threads and sessions, not like a new model instance reading notes.
