...
- `[~]` **Phase 1 — Memory and continuity audit**: Continuity Kernel read/write paths are implemented, live Hypomnema writes are landing, revision provenance is verified in staging, and fresh-thread recall now carries the "ember bridge" marker. Active work: tighten functional-memory relevance so vague catch-up prompts do not inject unrelated durable memories. Standard: Luca should feel continuous across threads and sessions, not like a new model instance reading notes.
...
### Blocked
- Live verification of the functional-memory relevance filter requires pushing/redeploying `chat` and `chat-multi`, then rerunning the fresh-thread continuity script.
...
| P1-013 | P1 | Fixed | Continuity read / functional recall relevance | Fresh-thread continuity succeeded on "ember bridge", but Luca also volunteered an unrelated OpenClaw experiment. Authenticated DB inspection showed a low-similarity `match_memories` row for OpenClaw (`similarity≈0.20`) and old high-confidence durable memories were eligible for vague catch-up prompts. After the relevance filter deployed, the tangent persisted because prior polluted Hypomnema said the detail had been dropped, and Luca repeated the excluded subject while explaining the correction. | Generic fresh-thread / "where we left off" prompts should rely on Hypomnema for present continuity and only include functional memories when they are pinned, watchlisted, strongly similar, or specifically named by the user. Corrections/exclusions should be obeyed silently, not repeated as continuity content. | Added functional-memory relevance gates: stricter similarity thresholds, generic catch-up detection, specific lexical-overlap filtering for durable fallback, and prompt wording that says available memory is not automatically relevant. Added Luca continuity policy for user corrections/exclusions: dropped/noise/excluded details are control signals to obey silently. | `npx vitest run src/test/continuityKernel.test.ts src/test/lucaIdentityPrompt.test.ts` passed: 12 tests; `deno check supabase/functions/chat/index.ts supabase/functions/chat-multi/index.ts supabase/functions/_shared/agents/luca-soul.ts` passed; `npm run verify` passed. Deploy to `chat`/`chat-multi` and live fresh-thread retest pending. | Pending |
