...
- `[~]` **Phase 1 — Memory and continuity audit**: Continuity Kernel read/write paths are implemented locally, live Hypomnema writes are now landing, and Mnemos recall works across fresh threads. Active work: fix revision provenance so revised Hypomnema entries point to the latest source turn before running the next fresh-thread carry test. Standard: Luca should feel continuous across threads and sessions, not like a new model instance reading notes.
...
### Blocked
- Live verification of the revision-provenance patch requires pushing/redeploying `hypomnema-write`, then rerunning the fresh-thread continuity script.
...
| P1-012 | P2 | Fixed | Hypomnema revision provenance | Live post-deploy test revised existing Hypomnema row `e184...` with "ember bridge recovery", but top-level `thread_id` and `source_message_id` still pointed to the older recall-failure turn. | Revised entries should point their current top-level provenance to the latest source turn while preserving old provenance inside the revision history. | Revision updates now set top-level `thread_id` / `source_message_id` from the current turn and store previous/current source IDs in `revisions` plus `meta.last_revision_source`. | `npx vitest run src/test/hypomnemaWrite.test.ts` passed: 3 tests; `deno check supabase/functions/hypomnema-write/index.ts supabase/functions/_shared/hypomnema/write.ts` passed; `npm run verify` passed. Live deploy/retest pending. | Pending |
