...
- `[~]` **Phase 2 — Core chat and agent experience**: Core sweep is implemented and locally verified across streaming, retry, permission cards, observer alcove, council mode, sub-agent reports, rich content, attachments, desktop, and mobile. Remaining verification gap: missing-key UX needs a no-key account or controlled no-key session smoke.
