# Agent Substrate Scoping SQL Proposal

Status: proposal only. Do not apply until approved.

This migration makes custom agents full Polyphonic residents instead of chat-only personas. Existing rows are backfilled to `luca`; future rows can be scoped to any user-owned agent.

```sql
-- Polyphonic full resident-agent substrate scoping.
-- Additive/backfill-first where possible. Drops user-only uniqueness only where
-- it prevents multiple agents from having their own state row.

BEGIN;

-- 1. Add agent_id to agent-owned memory, journal, Mnemos, and autonomy tables.
...
-- 2. Existing data belongs to Luca.
...
-- 3. Enforce defaults and not-null after backfill.
...
-- 4. Replace uniqueness that was one-row-per-user with one-row-per-user-agent.
DO $$
...
-- 5. Query indexes for agent-scoped pages and runtime retrieval.
...
-- 6. Agent-scoped functional-memory RPC.
...
-- 8. Agent-scoped vector Mnemos RPC.
CREATE OR REPLACE FUNCTION public.match_engrams_vector(
  query_embedding extensions.vector,
  match_count int DEFAULT 20,
  p_user_id uuid DEFAULT NULL,
  min_strength double precision DEFAULT 0,
  p_agent_id text DEFAULT 'luca'
)
RETURNS TABLE (
  id uuid,
...
GRANT EXECUTE ON FUNCTION public.match_engrams_vector(extensions.vector, int, uuid, double precision, text) TO authenticated, service_role;
...
WHERE table_schema = 'public'
  ...
  AND table_name IN (
    'memories','beliefs','journal_entries','engrams','connections','engram_archive',
    'mnemos_emotional_state','mnemos_digests','memory_candidates','emotional_state',
    'emotional_history','cognitive_state','thought_stream','memory_events',
    'entity_activity_log','activity_events','daily_logs','observer_logs','observer_notes',
    'thought_initiations','curiosity_questions','pending_revisions'
  )
ORDER BY table_name;
```
